...
function composeScores(scores) {
    return scores.map((score) => {
        return `select person_uuid, avg(${score.name}) as ${score.alias} 
        from photo_info where person_uuid = :person_id 
        and date >= :start_date and date <= :end_date
        group by person_uuid`;
}).join("\nunion all\n");
}
function getCurationScore(person_id, start_date, end_date) {
    query = `select
    person_uuid,
    ...
    and date >= :start_date and date <= :end_date
    group by person_uuid`;
    return txGetAll(query, {person_id, start_date, end_date})
};
