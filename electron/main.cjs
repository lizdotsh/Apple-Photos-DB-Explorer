...
function chooseSqlFile(db_path, sql_path) {
    try {
        ...
        logEmitter.emit(
            "log-update",
            `Library Version: ${in_res ? "17" : "16"}`,
        );
        const file = path.join(
            sql_path,
            in_res ? "gen_tables.sql" : "gen_tables_16.sql",
        );
...
async function handleFileAndDbActions(system_db_path, rollup_sql_path) {
    //, win) {
    try {
        logEmitter.emit("log-update", "Requesting Permissions...");
...
async function copyDatabase(system_photo_library_path) {
    try {
        ...
        if (perms === "denied") {
            logEmitter.emit(
                "error-update",
                "Permission denied. Please grant access to your Photos library in System Preferences.",
            );
            throw new Error(
                "Permission denied. Please grant access to your Photos library in System Preferences.",
            );
            ...
            logEmitter.emit(
                "log-update",
                "Deleting existing DB if it exists...",
            );
...
function readSqlFile(db_path, sql_path, rollup_sql_path) {
    try {
        ...
        logEmitter.emit(
            "log-update",
            "Running SQL script... may freeze up momentarily...",
        );
        // logEmitter.emit("log-update", sql_file);
        txExecAll(sql_file);
        ...
        logEmitter.emit(
            "log-update",
            "SQL script executed and database closed.",
        );
    } catch (err) {
